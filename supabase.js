...
async function signUp(phone, name, college) {
  await supabaseClient.from("users").upsert({ phone, name, college });
...
async function saveLead(data) {
  return supabaseClient.from("leads").insert({
    name: data.name,
    phone: data.phone,
...
async function savePurchase(phone, course, paymentId, amount) {
  return supabaseClient.from("purchases").insert({
    phone,
    course,
...
async function getPurchases(phone) {
  const { data } = await supabaseClient.from("purchases").select("*").eq("phone", phone);
  return data || [];
}

// Certificate verification
async function verifyCertificate(certId) {
  const { data } = await supabaseClient.from("certificates").select("*").eq("cert_id", certId).single();
  return data;
}
